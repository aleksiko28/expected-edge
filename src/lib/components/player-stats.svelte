<script lang="ts">
  ...
  function getFixtureDifficulty(team?: Team, isAway?: boolean) {
    ...
    if (team) {
      const homeDifficulty = isForward
        ? team.strength_defence_away
        : team?.strength_attack_away
      const awayDifficulty = isForward
...
<div class="py-6">
  ...
  <div class="flex gap-2 mt-4">
    {#if data}
      {#each getPlayerNextFixtures(data) as fixture}
        <div class="flex flex-col gap-2 items-center border p-2">
          <Badge
            size="small"
            code={getPlayerBadge(fixture.team_h, fixture.team_a)}
          />
          <span>{fixture.team_h === data.team ? "(H)" : "(A)"}</span>
          <Fdr
            difficulty={getOpponentStrength(fixture.team_h, fixture.team_a) ??
              1000}
          />
        </div>
      {/each}
